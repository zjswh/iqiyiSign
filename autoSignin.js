/*
cron "0 9 * * *" autoSignin.js, tag=爱奇艺签到
*/
...
function Checkin(remarks) {
    const errorMessage = [remarks]
    const _url = `https://vinfo.vip.iqiyi.com/external/vip_users?messageId=bf9bba83b4dc37efecd7b394d95064a9&appVersion=&lang=zh_cn
    &platform=b6c13e26323c537d&P00001=${P00001}&version=7.0&bizSource=qiyiV2_vip&vipTypes=1`
    return axios(_url)
        .then(data => {
            if(data.data.code !== "A00000") {
                errorMessage.push('获取用户详情失败' + data.data.message)
...
function SignIn(remarks) {
    ...
    const qyid = md5(stringRandom(16))
    const sign_date = {
        agentType: "1",
        agentversion: "1.0",
        appKey: "basic_pcw",
    ...
    let msg = ""
    const _url = 'https://community.iqiyi.com/openApi/task/execute?' + w(sign_date) + "&sign=" + sign
    return axios(_url, {
        headers: {
